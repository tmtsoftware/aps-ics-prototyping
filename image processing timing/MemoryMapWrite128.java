...
class MemoryMapWrite128 {
	...
	// file is assumed to be used over and over for each new frame
        ...
        for (int j=0; j<1000; j++) {
            ...
            delta[j] = System.nanoTime() - startTime;
        ...
        List overages = new ArrayList();
        for (int i=0; i<1000; i++) {
            total = total + delta[i];
            if (delta[i] > max) max = delta[i];
            if (delta[i] > 1000000) overages.add(i);
        }
        
        System.out.println("Write time average: " + (total/1000.0) + "ns,  max = " + max);
        
        System.out.println("trials over 1 ms : " + overages);
